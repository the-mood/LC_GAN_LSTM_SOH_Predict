...
def make_gan_dataset():
    ...
    for b in [b05, b06]:
        i = 0
        while i < 5851:
            dataset.append(list(b['CC_ratio'][i:i + 117]))
            i += 117
...
def train_for_generator_ic(dataset):
    ...
    # 创建生成器和判别器
    if len(os.listdir('./model/CC_ratio')) != 1:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        generator.load_weights('./model/CC_ratio/generator.ckpt')
        discriminator = Discriminator()
        discriminator.build(input_shape=(None, 117, 1))
        discriminator.load_weights('./model/CC_ratio/discriminator.ckpt')
    else:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        discriminator = Discriminator()
    ...
    for epoch in range(epochs):
        ...
        if epoch % 200 == 0:
            ...
            for i in range(0, 102):
                temp = pd.DataFrame(fake_data[i], columns=['CC_ratio'])
