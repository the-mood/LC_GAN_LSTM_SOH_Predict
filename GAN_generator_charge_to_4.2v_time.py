...
def make_gan_dataset():
    ...
    for b in [b05, b06]:
        i = 0
        while i < 5851:
            dataset.append(list(b['charge_to_4.2v_time'][i:i + 117]))
            i += 117
...
def train_for_generator_charge_time(dataset):
    ...
    # 创建生成器和判别器
    if len(os.listdir('./model/charge_to_4.2v_time')) != 1:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        generator.load_weights('./model/charge_to_4.2v_time/generator.ckpt')
        discriminator = Discriminator()
        discriminator.build(input_shape=(None, 117, 1))
        discriminator.load_weights('./model/charge_to_4.2v_time/discriminator.ckpt')
    else:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        discriminator = Discriminator()
    ...
    for epoch in range(epochs):
        ...
        if epoch % 200 == 0:
            ...
            for i in range(0, 102):
                temp = pd.DataFrame(fake_data[i], columns=['charge_to_4.2v_time'])
                g_data = pd.concat([g_data, temp], axis=0)
            # 将生成的数据存入csv
            g_data[['charge_to_4.2v_time']] \
                .to_csv('./data/generator_data/charge_to_4.2v_time/generator_data_%d.csv' % epoch,
                        index=False, header=['charge_to_4.2v_time'])
