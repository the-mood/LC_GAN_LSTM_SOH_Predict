...
def make_gan_dataset():
    ...
    for b in [b05, b06]:
        i = 0
        while i < 5851:
            dataset.append(list(b['discharge_to_min_voltage_time'][i:i + 117]))
            i += 117
...
def train_for_generator_tem(dataset):
    ...
    # 创建生成器和判别器
    if len(os.listdir('./model/discharge_to_min_voltage_time')) != 1:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        generator.load_weights('./model/discharge_to_min_voltage_time/generator.ckpt')
        discriminator = Discriminator()
        discriminator.build(input_shape=(None, 117, 1))
        discriminator.load_weights('./model/discharge_to_min_voltage_time/discriminator.ckpt')
    else:
        generator = Generator()
        generator.build(input_shape=(None, z_dim))
        discriminator = Discriminator()
    ...
    for epoch in range(epochs):
        ...
        if epoch % 200 == 0:
            ...
            for i in range(0, 102):
                temp = pd.DataFrame(fake_data[i], columns=['discharge_to_min_voltage_time'])
        ...
        if epoch % 1000 == 1 and epoch != 1:
            # print(d_losses)
            # print(g_losses)
            generator.save_weights('./model/discharge_to_min_voltage_time/generator_'+str(epoch)+'.ckpt',)
            discriminator.save_weights('./model/discharge_to_min_voltage_time/discriminator_'+str(epoch)+'.ckpt')
