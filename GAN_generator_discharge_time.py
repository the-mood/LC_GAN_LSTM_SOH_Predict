...
def train_for_generator_dis_time(dataset):
    ...
    # 创建生成器和判别器
    if len(os.listdir('./model/discharge_time')) != 1:
        generator.build(input_shape=(None, z_dim))
        generator.load_weights('./model/discharge_time/generator.ckpt')
        discriminator.build(input_shape=(None, 117, 1))
        discriminator.load_weights('./model/discharge_time/discriminator.ckpt')
    else:
        generator.build(input_shape=(None, z_dim))
        discriminator.build(input_shape=(None, 117, 1))
    ...
    for epoch in range(epochs):
        ...
        if epoch % 200 == 0:
            ...
            for i in range(0, 102):
                temp = pd.DataFrame(fake_data[i], columns=['discharge_time'])
        ...
        if epoch % 1000 == 1 and epoch != 1:
            # print(d_losses)
            # print(g_losses)
            generator.save_weights('./model/discharge_time/generator_' + str(epoch) + '.ckpt', )
            discriminator.save_weights('./model/discharge_time/discriminator_' + str(epoch) + '.ckpt')
